Raqcoin-Qt: Qt5 GUI for Raqcoin
===============================
...
An executable named `raqcoin-qt` will be built.
...
To use UPnP for port forwarding behind a NAT router (recommended, as more connections overall allow for a faster and more stable raqcoin experience), pass the following argument to qmake:
...
A warning for people using the *static binary* version of Raqcoin on a Linux/UNIX-ish system (tl;dr: **Berkely DB databases are not forward compatible**).

The static binary version of Raqcoin is linked against libdb (see also `this Debian issue`_).
...
installed causes raqcoin-qt to crash intermittently.  The issue has been reported as `launchpad bug 857790`_, but
isn't yet fixed.
