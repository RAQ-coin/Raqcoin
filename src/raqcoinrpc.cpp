...
{
    return GetBoolArg("-testnet", false) ? 18882 : 8882;
}
...
int64 AmountFromValue(const Value& value)
{
    ...
    if (dAmount <= 0.0 || dAmount > 2147483647.0)
...
Value stop(const Array& params, bool fHelp)
{
    ...
    StartShutdown();
    return "Raqcoin server stopping";
}
...
{ //  name                      actor (function)         okSafeMode threadSafe
  //  ------------------------  -----------------------  ---------- ----------
    ...
    { "getpublickeypos",        &getpublickeypos,        false,     false },
...
string HTTPPost(const string& strMsg, const map<string,string>& mapRequestHeaders)
{
    ostringstream s;
    s << "POST / HTTP/1.1\r\n"
      << "User-Agent: raqcoin-json-rpc/" << FormatFullVersion() << "\r\n"
      << "Host: 127.0.0.1\r\n"
...
//
// JSON-RPC protocol.  Raqcoin speaks version 1.0 for maximum compatibility,
                   ...
                   const bool fUseSSL)
{
    // Accept connection
#if BOOST_VERSION >= 107000
    auto& io_ctx = static_cast<asio::io_context&>(acceptor->get_executor().context());
    AcceptedConnectionImpl<Protocol>* conn = new AcceptedConnectionImpl<Protocol>(io_ctx, context, fUseSSL);
#else
...
void StartRPCThreads()
{
        ...
        (mapArgs["-rpcuser"] == mapArgs["-rpcpassword"]))
    {
        ...
        string strWhatAmI = "To use raqcoin";
...
Array RPCConvertValues(const std::string &strMethod, const std::vector<std::string> &strParams)
{
    ...
    if (strMethod == "importkey"              && n > 2) ConvertTo<bool>(params[2]);
