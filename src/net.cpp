...
void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "Raqcoin " + FormatFullVersion();
...
//unsigned int pnSeed[] ={0xD3471A78,0x60B66827, 0x02316827, 0xC3326827};
...
std::vector<SeedSpec6> vFixedSeeds = std::vector<SeedSpec6>(pnSeed6_main, pnSeed6_main + ARRAYLEN(pnSeed6_main));
    ...
    while(true)
    {
        ...
        if (addrman.size()==0 && (GetTime() - nStart > 60) && !fTestNet)
        {
            ...
            if (!done) {
                ...
                addrman.Add(convertSeed6(vFixedSeeds), local);
                done = true;
            }
        }
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. Raqcoin is probably already running."), addrBind.ToString().c_str());
        else
