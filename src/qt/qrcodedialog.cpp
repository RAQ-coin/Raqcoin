...
void QRCodeDialog::setModel(OptionsModel *model)
{
    ...
    // update the display unit, to not use the default ("ABC")
    updateDisplayUnit();
}
...
{
    QString ret = QString("abcmint:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non ABC unit input in lnReqAmount, we generate the URI with ABC as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(RaqcoinUnits::format(RaqcoinUnits::ABC, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
