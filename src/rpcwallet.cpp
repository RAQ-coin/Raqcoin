...
Value getnewaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            ...
            "Returns a new Raqcoin address for receiving payments.  "
    ...
    if(pwalletMain->GetKeyFromPool(newKey, config_value, false)) {
        ...
        return CRaqcoinAddress(keyID).ToString();
    } else
    	throw runtime_error("GetKeyFromPool return false\n");
}


CRaqcoinAddress GetAccountAddress(string strAccount, bool bForceNew=false)
{
    ...
    return CRaqcoinAddress(account.vchPubKey.GetID());
}
...
/*Value getaccountaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "Returns the current Raqcoin address for receiving payments to this account.");
...
Value setaccount(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 2)
        throw runtime_error(
            "setaccount <raqcoinaddress> <account>\n"
            "Sets the account associated with the given address.");

    CRaqcoinAddress address(params[0].get_str());
    if (!address.IsValid())
...
Value getaddressesbyaccount(const Array& params, bool fHelp)
{
    ...
    Array ret;
    BOOST_FOREACH(const PAIRTYPE(CRaqcoinAddress, string)& item, pwalletMain->mapAddressBook)
    {
...
Value sendtoaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 4)
        throw runtime_error(
            "sendtoaddress <raqcoinaddress> <amount> [comment] [comment-to]\n"
...
Value listaddressgroupings(const Array& params, bool fHelp)
{
    ...
    BOOST_FOREACH(set<CTxDestination> grouping, pwalletMain->GetAddressGroupings())
    {
        ...
        {
            Array addressInfo;
            addressInfo.push_back(CRaqcoinAddress(address).ToString());
            addressInfo.push_back(ValueFromAmount(balances[address]));
            {
                LOCK(pwalletMain->cs_wallet);
                if (pwalletMain->mapAddressBook.find(CRaqcoinAddress(address).Get()) != pwalletMain->mapAddressBook.end())
                    addressInfo.push_back(pwalletMain->mapAddressBook.find(CRaqcoinAddress(address).Get())->second);
            }
...
Value signmessage(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 2)
        throw runtime_error(
            "signmessage <raqcoinaddress> <message>\n"
    ...
    CRaqcoinAddress addr(strAddress);
    if (!addr.IsValid())
...
Value verifymessage(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            "verifymessage <raqcoinaddress> <signature> <message>\n"
...
Value getreceivedbyaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            "getreceivedbyaddress <raqcoinaddress> [minconf=1]\n"
            "Returns the total amount received by <raqcoinaddress> in transactions with at least [minconf] confirmations.");

    // Raqcoin address
    CRaqcoinAddress address = CRaqcoinAddress(params[0].get_str());
...
Value newgetbalance(const Array& params, bool fHelp)
{
    ...
    if (params.size() != 0) {
        ...
        if (!CRaqcoinAddress(strAddress).IsValid())
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid Raqcoin address");
    }
...
Value sendfrom(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 3 || params.size() > 6)
        throw runtime_error(
            "sendfrom <fromaccount> <toraqcoinaddress> <amount> [minconf=1] [comment] [comment-to]\n"
            "<amount> is a real and is rounded to the nearest 0.00000001"
            + HelpRequiringPassphrase());

    string strAccount = AccountFromValue(params[0]);
    CRaqcoinAddress address(params[1].get_str());
    if (!address.IsValid())
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid Raqcoin address");
    int64 nAmount = AmountFromValue(params[2]);
    int nMinDepth = 1;
...
Value sendmany(const Array& params, bool fHelp)
{
    ...
    set<CRaqcoinAddress> setAddress;
    vector<pair<CScript, int64> > vecSend;

    int64 totalAmount = 0;
    BOOST_FOREACH(const Pair& s, sendTo)
    {
        CRaqcoinAddress address(s.name_);
        if (!address.IsValid())
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, string("Invalid Raqcoin address: ")+s.name_);
...
static CScript _createmultisig(const Array& params)
{
    ...
    std::vector<CDiskPubKeyPos> vPubKeyPos;
    vPubKeyPos.resize(keys.size());
    for (unsigned int i = 0; i < keys.size(); i++)
    {
        const std::string& ks = keys[i].get_str();
        if (IsHex(ks) && ks.length() == RAINBOW_PUBLIC_KEY_POS_SIZE*2)
        {
            CDiskPubKeyPos pos;
            pos<< ParseHex(ks);
            CPubKey vchPubKey;
            bool ret = GetPubKeyByPos(pos, vchPubKey);
            if (!ret || !vchPubKey.IsValid())
                throw runtime_error(" can't find public key at position: "+ks);
            vPubKeyPos[i] = pos;
        }
        else
        {
            throw runtime_error(" Invalid public key position: "+ks);
        }
    }
    CScript result;
    result.SetMultisig(nRequired, vPubKeyPos);
    return result;
}

Value getpublickeypos(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1 )
    {
        string msg = "getpublickeypos <address>\n"
            "the raqcoin public key is too large, translate it to position in the block chain by this interface \n"
    ...
    std::string addressStr = CRaqcoinAddress(pubKey.GetID()).ToString();
...
Value addmultisigaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 3)
    {
        string msg = "addmultisigaddress <nrequired> <'[\"keypos\",\"keypos\"]'> [account]\n"
            ...
            "each keypos is a Raqcoin public key position, you can get the position by getpublickeypos\n"
    ...
    return CRaqcoinAddress(innerID).ToString();
}
...
Value createmultisig(const Array& params, bool fHelp)
{
    ...
    CScript inner = _createmultisig(params);
    CScriptID innerID = inner.GetID();
    CRaqcoinAddress address(innerID);
...
Value ListReceived(const Array& params, bool fByAccounts)
{
    ...
    // Tally
    map<CRaqcoinAddress, tallyitem> mapTally;
    for (map<uint256, CWalletTx>::iterator it = pwalletMain->mapWallet.begin(); it != pwalletMain->mapWallet.end(); ++it)
    {
        const CWalletTx& wtx = (*it).second;
    ...
    BOOST_FOREACH(const PAIRTYPE(CRaqcoinAddress, string)& item, pwalletMain->mapAddressBook)
    {
        ...
        map<CRaqcoinAddress, tallyitem>::iterator it = mapTally.find(address);
...
void ListTransactions(const CWalletTx& wtx, const string& strAccount, int nMinDepth, bool fLong, Array& ret)
{
    ...
    if ((!listSent.empty() || nFee != 0) && (fAllAccounts || strAccount == strSentAccount))
    {
        BOOST_FOREACH(const PAIRTYPE(CTxDestination, int64)& s, listSent)
        {
            ...
            entry.push_back(Pair("address", CRaqcoinAddress(s.first).ToString()));
    ...
    if (listReceived.size() > 0 && wtx.GetDepthInMainChain() >= nMinDepth)
    {
        BOOST_FOREACH(const PAIRTYPE(CTxDestination, int64)& r, listReceived)
        {
            ...
            if (fAllAccounts || (account == strAccount))
            {
                ...
                entry.push_back(Pair("address", CRaqcoinAddress(r.first).ToString()));
                if (wtx.IsCoinBase())
                {
...
Value encryptwallet(const Array& params, bool fHelp)
{
    ...
    StartShutdown();
    return "wallet encrypted; Raqcoin server stopping, restart to run with encrypted wallet. The keypool has been flushed, you need to make a new backup.";
}
...
public:
    ...
    Object operator()(const CScriptID &scriptID) const {
        ...
        BOOST_FOREACH(const CTxDestination& addr, addresses)
            a.push_back(CRaqcoinAddress(addr).ToString());
...
Value validateaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "validateaddress <raqcoinaddress>\n"
            "Return information about <raqcoinaddress>.");
